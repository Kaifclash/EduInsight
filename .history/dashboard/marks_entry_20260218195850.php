...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $selectedClassPost = (int)($_POST["class_id"] ?? 0);

  if ($exam_id<=0 || $student_id<=0 || $subject_id<=0 || $selectedClassPost<=0) {
    header("Location: marks_entry.php?req=1&class_id=".$selectedClassPost); exit;
  }
  if ($max<=0 || $marks<0 || $marks>$max) {
    header("Location: marks_entry.php?bad=1&class_id=".$selectedClassPost); exit;
  }
  ...
  if ($exists) {
    ...
    header("Location: marks_entry.php?upd=1&class_id=".$selectedClassPost); exit;
  } else {
    ...
    header("Location: marks_entry.php?ok=1&class_id=".$selectedClassPost); exit;
  }
}
...
<title>EduInsight | Teacher Marks Entry</title>
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .btn{border:1px solid var(--border);background:rgba(255,255,255,.06);color:var(--text);padding:10px 12px;border-radius:12px;font-weight:900;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:8px;}
...
<div class="wrap">
  <div class="top">
    <div>
      <b style="font-size:18px;">Teacher Marks Entry</b>
      <div style="color:var(--muted);font-weight:700;font-size:13px;margin-top:4px;">
        Select class → student → subject → exam
      </div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
      <a class="btn" href="teacher_dashboard.php">← Teacher Dashboard</a>
  ...
  <div class="card">
    <form method="GET" class="grid">
      <div>
        <label>Choose Class (loads students & subjects)</label>
        <div class="field">
